...
with open("highscore.txt", 'r+') as highscore_file:
  ...
  print(highscore, 'highscore after')
...
def play_game(playing):
  while playing:
      which_mode = get_valid_num("Do you wish to play endless mode or learn a new song? type '1' for endless, '2' for new song or 3 to quit: ", 3)
      if which_mode == 1:
...
def play_song(mode, song_notes = '', note_speed = 0, chosen_lives = 10, chosen_max = None):
  ...
  while playing_notes:
    while key_is_falling:
      ...
      if mode == 'endless':
        ...
        if y >= 400:
          ...
          if tiles_fallen == 0:
            ...
            screen.blit(start_message, (20,10))
            y = y
          else:
            screen.fill((189, 151, 30))
      ...
      if mode == 'endless':
        ...
        screen.blit(score_message, (20, 85))
        if score > highscore:
          highscore = score
